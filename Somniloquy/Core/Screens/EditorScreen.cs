namespace Somniloquy {
    ...
    public class EditorScreen : Screen {
        ...
        public ColorPicker ColorChart { get; private set; } = null;
        ...
        public EditorScreen(Rectangle boundaries) : base(boundaries) {
            ...
            ColorChart = new ColorPicker(new Rectangle(SQ.WindowSize.X - 144, SQ.WindowSize.Y - 144, 128, 128), this);
        ...
        public override void Update() {
            ...
            if (InputManager.IsKeyDown(Keys.LeftControl) && InputManager.IsKeyPressed(Keys.L)) {
                if (InputManager.IsKeyDown(Keys.LeftShift)) {
                    var path = Dialog.FileOpen("png", SQ.CM.RootDirectory).Path;

                    if (File.Exists(path)) {
                        WorldScreen.LoadToWorld(SQ.LoadImage(path));
                    }

                    SQ.FocusWindow();
                }
                else {
                    var path = Dialog.FileOpen("txt", SQ.CM.RootDirectory).Path;
            ...
            if (InputManager.IsKeyDown(Keys.LeftControl) && InputManager.IsKeyPressed(Keys.S)) {
                ...
                var path = Dialog.FileSave("txt", SQ.CM.RootDirectory).Path;
